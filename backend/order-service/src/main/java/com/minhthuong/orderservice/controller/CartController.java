...
import com.minhthuong.orderservice.http.header.HeaderGenerator;
import com.minhthuong.orderservice.service.CartService;
...
public class CartController {
    ...
    private String getCartIdFromCookies(HttpServletRequest request, HttpServletResponse response) {
        log.info("=== Debugging Cookie Information ===");
        // 1. Ưu tiên lấy từ cookie header
        ...
        // 2. Nếu không có, lấy từ mảng cookies
        ...
        // 3. Nếu vẫn không có, thử lấy từ query param
        ...
        // 4. Nếu vẫn không có, tự động tạo mới cartId và set vào response (chỉ khi thao tác thêm vào giỏ hàng)
        String newCartId = java.util.UUID.randomUUID().toString();
        Cookie newCookie = new Cookie("cartId", newCartId);
        newCookie.setPath("/");
        newCookie.setHttpOnly(false); // Cho phép JS lấy được nếu cần
        newCookie.setMaxAge(60 * 60 * 24 * 30); // 30 ngày
        response.addCookie(newCookie);
        log.info("Tạo mới cartId: {} và set vào cookie", newCartId);
        return newCartId;
    }
    ...
    public ResponseEntity<CartResponse> getCart(HttpServletRequest request, HttpServletResponse response) {
        ...
        String cartId = getCartIdFromCookies(request, response);
        if (cartId == null) {
