...
const UserLogin = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // Xử lý nhiều trường hợp trả về token từ backend
            let token = '';
            ...
            if (token) {
                // Đảm bảo token luôn có prefix 'Bearer '
                if (!token.startsWith('Bearer ')) {
                    token = 'Bearer ' + token;
                }
                localStorage.setItem('authToken', token);
                console.log('Token đã lưu vào localStorage:', localStorage.getItem('authToken'));
